...
const IgnoreCircle = ({ svg = false, image = false, to, tab_name, onClick, className }) => {
      const TagName = onClick ? 'div' : Link
     
      if (svg) {
            return (
                  <ShowTabName className={`cursor-pointer ${className}`} tabName={tab_name} onClick={onClick}>
   ...
   return (
      <TagName className={cx("rounded-full overflow-hidden w-10 h-10 mx-1 cursor-pointer", `${className}`)} onClick={onClick}>
